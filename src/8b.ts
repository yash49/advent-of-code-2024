...
function generateAntinode(
  ...
  while (
    ...
    if (map[antinode1.i][antinode1.j] === '.')
      map[antinode1.i][antinode1.j] = '#';

    antinode1.i = antinode1.i + x;
    antinode1.j = antinode1.j + y;
  }
  ...
  while (
    ...
    if (map[antinode2.i][antinode2.j] === '.')
      map[antinode2.i][antinode2.j] = '#';

    antinode2.i = antinode2.i - x;
    antinode2.j = antinode2.j - y;
  }
}
...
async function solve() {
  ...
  const antinodesMap = inputArr.split('\n').map((line) => line.split(''));
